...
const VideoProcessor = () => {
  ...
  const loadDevices = async () => {
    try {
      ...
      console.log(
        `[DEVICES] Found ${videoDevices.length} video devices:`,
        videoDevices
      );
      ...
      if (videoDevices.length > 0 && !currentDeviceId) {
        ...
        console.log(
          `[DEVICES] Setting default deviceId: ${videoDevices[0].deviceId}`
        );
  ...
  const switchCamera = async () => {
    ...
    if (isMobileDevice()) {
      ...
      console.log(
        `[DEVICES] Switching from ${currentFacingMode} to ${newFacingMode} camera`
      );
    ...
    console.log(
      `[DEVICES] Switching from device ${currentIndex} (${currentDeviceId}) to ${nextIndex} (${nextDevice.deviceId})`
    );
  ...
  const startStream = async (facingMode?: string) => {
    try {
      ...
      if (isFirstStart) {
        console.log(
          "[WEBCAM] First start - attempting to use environment-facing camera"
        );
      ...
      if (isFirstStart) {
        ...
        if (streamTrack) {
          ...
          if (settings.deviceId) {
            console.log(
              `[WEBCAM] Updating current device ID to: ${settings.deviceId}`
            );
  ...
  const getVideoTensor = (
    ...
    console.log(
      `[TENSOR] getVideoTensor called. Video: ${width}x${height}, lowQuality: ${lowQuality}`
    );
    ...
    return tf.tidy(() => {
      ...
      if (lowQuality && width > 320 && height > 240) {
        ...
        console.log(
          `[TENSOR]   Downscaling from ${width}x${height} to ${newWidth}x${newHeight}`
        );

        // Resize the tensor to lower resolution
        const resizedTensor = tf.image.resizeBilinear(fullResTensor, [
          newHeight,
          newWidth,
        ]) as tf.Tensor3D;
        console.log(
          `[TENSOR]   Resized tensor shape: [${resizedTensor.shape}]`
        );
        return resizedTensor;
      } else {
        console.log(
          `[TENSOR]   Using full resolution tensor (lowQuality=${lowQuality}, width=${width}, height=${height})`
        );
  ...
  const processFrame = () => {
    ...
    if (!resultCtx) {
      console.error(
        "[FRAME] Failed to get 2D context from result canvas. Stopping processing."
      );
    ...
    (async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            `[FRAME] Setting result canvas dimensions to match tensor: ${tensorWidth}x${tensorHeight}`
          );
          resultCanvas.width = tensorWidth;
          resultCanvas.height = tensorHeight;
        }
        ...
        if (enhanceContrastEnabled) {
          ...
          const currentResultCtx = resultCanvas.getContext("2d", {
            willReadFrequently: true,
          });
          if (currentResultCtx) {
            const imageData = currentResultCtx.getImageData(
              0,
              0,
              resultCanvas.width,
              resultCanvas.height
            ...
            console.warn(
              "[ENHANCE] Could not get context after resize for enhancement"
            );
          }
        }
        ...
        <div className="flex-1 space-y-4">
          <div className="bg-gray-800 p-4 rounded-lg shadow-lg">
            ...
            <div className="aspect-video bg-gray-700 rounded flex items-center justify-center overflow-hidden relative">
              ...
              {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-70">
                  <p className="text-red-500 bg-gray-900 p-4 rounded-lg text-sm">
                    Error: {error}
                  </p>
                </div>
              )}
      ...
      <div className="mt-8 bg-gray-800 p-4 rounded-lg shadow-lg">
        ...
        <div className="mt-4 flex gap-4">
          {isStreaming ? (
            <button
              onClick={stopStream}
              className={`px-4 py-2 text-white rounded shadow transition-colors ${
                isProcessing
