...
const BenchmarkPage = () => {
      ...
      try {
        ...
        for (const imageName of imageNames) {
          ...
          try {
            ...
            console.log(
              `[BENCHMARK] Successfully loaded ${imageName} from: ${path}`
            );
          } catch (error) {
            console.error(
              `[BENCHMARK] Failed to load ${imageName} from: ${path}. Ensure the image exists in the /public directory.`
            );
          }
        }
  ...
  const runBenchmark = async () => {
    ...
    try {
      ...
      for (const [imageName, img] of Object.entries(imagesRef.current)) {
        ...
        const tfResults = await benchmarkTensorflowJS(
          img,
          iterations,
          useCache
        );
  ...
  const benchmarkTensorflowJS = async (
    ...
    console.log(
      `[BENCHMARK-TFJS] Avg Time: ${averageTime.toFixed(
        2
      )}ms, FPS: ${fps.toFixed(1)}`
    );
  ...
  const benchmarkSobelTS = async (
    ...
    if (validTimes.length === 0 && times.length > 0) {
      ...
      console.warn(
        `[BENCHMARK-TS] ${times.length - validTimes.length} iterations failed.`
      );
    }

    const totalTime = validTimes.reduce((acc, time) => acc + time, 0);
    const averageTime =
      validTimes.length > 0 ? totalTime / validTimes.length : NaN;
    const fps = averageTime > 0 ? 1000 / averageTime : NaN;

    console.log(
      `[BENCHMARK-TS] Avg Time: ${averageTime.toFixed(2)}ms, FPS: ${fps.toFixed(
        1
      )}`
    );
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6">
        Sobel Filter Performance Benchmark
      </h1>
      <p className="mb-6 text-gray-300">
        This benchmark compares the performance of the TensorFlow.js-based Sobel
        filter implementation against the{" "}
        <code className="bg-gray-800 px-1 rounded">sobel-ts</code> library (a
      ...
      {loading && (
        <div className="bg-gray-900 rounded-lg p-6 mb-6 border border-gray-800">
          ...
          <p className="text-gray-400">
            {currentTest || "Preparing benchmark..."}
          </p>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {results.map((result, index) => {
                  ...
                  const pairedResult = results.find(
                    (r) =>
                      r.imageName === result.imageName &&
                      r.library !== result.library
                  );
                  ...
                  if (pairedResult) {
                    performanceRatio =
                      pairedResult.executionTime / result.executionTime;
                    ...
                    >
                      ...
                      <td className="p-3">
                        {pairedResult && (
                          <span
                            className={`${
                              isFaster ? "text-green-400" : "text-yellow-400"
                            }`}
                          >
                            {isFaster
          ...
          <div className="mt-6 text-gray-400 text-sm">
            <p>
              Benchmark run with {iterations} iterations per test{" "}
              {useCache ? "with" : "without"} tensor caching.
            </p>
            <p className="mt-2">
              <strong>Note:</strong> Results may vary based on hardware,
              browser, and system load. First-time runs may be slower due to
            ...
            <div className="mt-4 p-4 bg-gray-800 rounded">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>
                  <span className="text-blue-400 font-medium">sobel-tfjs</span>{" "}
                  uses TensorFlow.js to process images with GPU acceleration
                  where available.
                </li>
                <li>
                  <span className="text-green-400 font-medium">sobel-ts</span>{" "}
                  uses pure JavaScript code running on the CPU for the same
                  algorithm.
                </li>
                <li>
                  The difference between them demonstrates the performance
                  advantage of hardware-accelerated machine learning libraries.
                </li>
                <li>
                  Larger performance differences generally indicate better GPU
                  utilization.
                </li>
              </ul>
            </div>
