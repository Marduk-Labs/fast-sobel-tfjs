...
const ImageProcessor = () => {
    ...
    const processImage = async () => {
        ...
        try {
            ...
            if (!resultCanvas) {
                ...
                alert('Error: Result canvas not available. Please try reloading the page.')
                return
            }
            ...
            console.log("Original Image data dimensions:", imageData.width, "x", imageData.height);
            ...
            console.log(`Original image data sum: ${origSum}, length: ${origDataArray.length}`);

            if (origSum === 0) {
                console.error("ERROR: Original image data is all zeros. Canvas might be empty.");
                alert("The canvas appears to be empty. Please try reloading the image.");
            ...
            try {
                ...
                console.log(`Processed image data sum (BEFORE post-processing): ${sum}`);

                // Compute some statistics
                let min = 255, max = 0, nonZeroPixels = 0;
                ...
                console.log(`Image stats (BEFORE post-proc) - Min: ${min}, Max: ${max}, Non-zero pixels: ${nonZeroPixels} (${(nonZeroPixels / (dataArray.length / 4) * 100).toFixed(2)}%)`);

                if (sum === 0) {
                    console.warn("WARNING: All pixel values are 0. The result may appear black.");
                    alert("The processed image appears to be black. Please try a different image or settings.");
                    setIsProcessing(false);
                    return;
                }
                ...
                if (scale !== 1.0 && scale !== 0) {
                    ...
                    if (tempCtx) {
                        ...
                        if (scaledCtx) {
                            ...
                            // Get the result data
                            resultImageData = scaledCtx.getImageData(0, 0, canvas.width, canvas.height)
                ...
                console.log(`Final ImageData dimensions: ${resultImageData.width}x${resultImageData.height}`);
                ...
                if (tempCtx) {
                    ...
                    testImg.onload = () => console.log('Validation image loaded successfully');
                    testImg.onerror = (err) => console.error('Validation image failed to load:', err);
                    testImg.src = tempUrl;
                }
                ...
                if (resultCanvas.width !== resultImageData.width || resultCanvas.height !== resultImageData.height) {
                    console.log(`Resizing result canvas from ${resultCanvas.width}x${resultCanvas.height} to ${resultImageData.width}x${resultImageData.height}`);
                ...
                const premultipliedData = new Uint8ClampedArray(resultImageData.data.length);
                ...
                for (let i = 0; i < resultImageData.data.length; i += 4) {
                    ...
                    premultipliedData[i + 1] = Math.round(resultImageData.data[i + 1] * alpha);
                    premultipliedData[i + 2] = Math.round(resultImageData.data[i + 2] * alpha);
                    premultipliedData[i + 3] = resultImageData.data[i + 3];
                }
                console.log(`Premultiplied alpha for ${nonZeroAlphaCount} pixels with non-zero alpha.`);
                const premultipliedImageData = new ImageData(premultipliedData, resultImageData.width, resultImageData.height);
                // --- End Premultiply Alpha ---

                // --- Additional Debugging --- 
                console.log('First 40 pixels of PREMULTIPLIED ImageData:',
                ...
                if (testCtx) {
                    try {
                        testCtx.putImageData(
                            new ImageData(
                                new Uint8ClampedArray(premultipliedImageData.data.slice(0, 400)),
                                10,
                                10
                            ),
                            0,
                        ...
                        console.log('Test canvas (10x10) URL:', testCanvas.toDataURL().substring(0, 100) + "...");
                    } catch (e) {
                ...
                console.log("Clearing and drawing PREMULTIPLIED data to main canvas...");
                resultCtx.clearRect(0, 0, resultCanvas.width, resultCanvas.height);
                resultCtx.putImageData(premultipliedImageData, 0, 0); // Use premultiplied data

                // Verify the drawn data
                try {
                    const checkData = resultCtx.getImageData(0, 0, resultCanvas.width, resultCanvas.height);
                    const checkSum = Array.from(checkData.data).reduce((sum, val) => sum + val, 0);
                    console.log(`Main canvas sum (AFTER drawing PREMULTIPLIED): ${checkSum}`);
                } catch (readbackError: any) {
                    console.warn(`Error reading back main canvas: ${readbackError.message}`);
                }
                ...
                console.log('Setting result image URL, canvas dimensions:', resultCanvas.width, 'x', resultCanvas.height);
    ...
    useEffect(() => {
        if (resultImageUrl) {
            console.log('Result image URL updated:', resultImageUrl.substring(0, 50) + '...');
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Kernel Size</label>
                                <select
                                    value={kernelSize}
                            ...
                            <div className="flex items-center">
                                ...
                                />
                                <label htmlFor="grayscale" className="text-gray-300">Convert to grayscale</label>
                            </div>
                            ...
                            <div className="flex items-center">
                                <input
                                    ...
                                    className="mr-2 h-4 w-4"
                                />
                                <label htmlFor="enhanceContrast" className="text-gray-300">Enhance contrast</label>
                            </div>
                            ...
                            <button
                                ...
                                className={`w-full py-3 px-4 rounded font-medium transition-colors ${!originalImage || isProcessing
                    ...
                    <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div>
                                {resultImageUrl && (
                                    <button
                                        onClick={() => downloadImage(resultCanvasRef.current, 'edge-detection-tfjs.png')}
                                        className="text-accent hover:text-accent-light"
                                    >
                                        Download Result
                                    </button>
